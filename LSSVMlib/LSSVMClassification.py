...
class LSSVMClassification(BaseEstimator, ClassifierMixin):
    ...
    def get_params(self, deep=True):
        """
            The get_params functionality provides the parameters of the LSSVMClassification class.
    ...
    def __optimize_parameters(self, X, y):
        """
        ...
        # Classification
        #Omega = np.multiply( np.multiply.outer(y.T,y.T), self.kernel_(self.x,self.x) ) 
        Omega = np.multiply( y*y.T, self.KxX ) 
        D = Omega + self.gamma**-1 * np.identity(y.size)
        # correct version
        ...
        A_dag = np.linalg.pinv(A) #need to check if the matrix is OK--> y.T parts
        B = np.array([0]+[1]*len(self.y))
        #Regression
    ...
    def fit(self, X: np.ndarray, y: np.ndarray):
        ...
        #checks if X is an instance of either types
        Xloc = X.to_numpy() if isinstance(X, (pd.DataFrame, pd.Series)) else X
        yloc = y.to_numpy() if isinstance(y, (pd.DataFrame, pd.Series)) else y
        ...
        self.KxX = self.kernel_(self.sv_x, X_reshaped)            
        ...
        if len(self.y_labels) == 2: # binary classification       
            ...
            self.b, self.alpha = self.__optimize_parameters(X, y_values)     
            ...
            for i in range(n_classes):
                ...
                self.b[i], self.alpha[i] = self.__optimize_parameters(X, y_values)    
    ...
    def predict(self, X: np.ndarray) -> np.ndarray:
        ...
        X_reshaped = X.reshape(1,-1) if X.ndim==1 else X
        KxX = self.kernel_(self.sv_x, X_reshaped)
        ...
        if len(self.y_labels)==2: # binary classification
            ...
            return np.where(y==-1, self.y_labels[0], self.y_labels[1])
            ...
            return np.array([self.y_labels[i] for i in predictions])
